...
77. CSP `unsafe-eval` console error — Sentry Session Replay uses `new Function()` internally, blocked by production CSP; disabled replay (set both sample rates to 0); also added `challenges.cloudflare.com` to CSP `script-src` for Turnstile

## Session 2026-02-15 (batch 23)
78. `use-offline-queue.ts` `syncAll` missing try-catch-finally — if `getPendingSubmissions()`, `syncSubmission()`, or `loadPendingSubmissions()` throws, `isSyncingRef.current` stays `true` forever, permanently locking the offline queue until page reload (same pattern as bugs #70/#74, different file)
79. `use-general-chat.ts` `toggleLike` and `reportMessage` depend on `messages` in useCallback deps — `messages` changes on every realtime chat message, causing both callbacks to be recreated on every incoming message, triggering re-render churn in consuming components; `reportMessage` fallback also reads potentially stale `report_count` — switched to `messagesRef.current` to read latest state without dep
80. `modal.tsx` scroll lock interference between stacked modals — `ConfirmModal` renders a `<Modal>` inside it; when ConfirmModal closes over a still-open parent Modal, `document.body.style.overflow` resets to `''`, making page scrollable behind the parent modal — added module-level `openModalCount` so scroll only unlocks when all modals close
81. `favorites-modal.tsx` bare `createClient()` (line 47) creates new Supabase client on every render — wrapped in `useMemo(() => createClient(), [])` (same pattern as bugs #43, #59, #60, #65, #67)
82. `admin/tournaments/page.tsx` `openTeamsModal` (lines 336-350) missing error handling on tournament_teams query — if fetch fails, stale teams from previous tournament remain in state; added try-catch with toast error + `setTournamentTeams([])` to clear stale data on open
83. `admin/moderation/page.tsx` `toLocaleString()` (line 396) missing `timeZone: 'Pacific/Honolulu'` — moderation timestamps display in browser timezone instead of Hawaii time (same pattern as bugs #71-76)
