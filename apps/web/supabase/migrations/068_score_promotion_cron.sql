...
BEGIN
  ...
  -- Determine new game status based on submission type
  IF v_submission.submission_type = 'final_score' THEN
    v_new_status := 'final';
  ELSE
  ...
  SET
    status = 'published',
    promoted_at = NOW(),
    promotion_reason = p_promotion_type
  WHERE id = p_submission_id;

  -- Update the game (NOT verified for auto-promotion)
  UPDATE games
  SET
    ...
    is_verified = FALSE,  -- Auto-promoted scores are NOT verified
    official_submission_id = p_submission_id,
    last_score_update_at = NOW(),
    verification_method = p_promotion_type
  WHERE id = v_submission.game_id;

  -- Mark other pending submissions as superseded
  UPDATE submissions
  SET status = 'rejected'
  WHERE game_id = v_submission.game_id
    AND status = 'pending'
  ...
  INSERT INTO score_promotion_log (
    ...
    p_submission_id,
    p_promotion_type,
    v_submission.home_score,
    v_submission.away_score,
    p_promoted_by_user_id,
    v_game.home_score,
    v_game.away_score,
    v_game.official_submission_id,
    p_metadata
  );
...
BEGIN
  ...
  INSERT INTO score_promotion_log (
    ...
    p_game_id,
    COALESCE(v_game.official_submission_id, '00000000-0000-0000-0000-000000000000'::UUID),
    'manual_verification',
    v_game.home_score,
    v_game.away_score,
    p_user_id,
...
BEGIN
  ...
  IF v_game.official_submission_id IS NOT NULL THEN
    ...
    WHERE id = v_game.official_submission_id;
  END IF;
