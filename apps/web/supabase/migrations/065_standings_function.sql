-- ============================================
-- Migration 065: Standings Computation Function
-- ============================================
-- This migration creates the get_computed_standings function that:
-- 1. Uses teams table for proper league/division/region grouping
-- 2. Calculates standings from games (not pre-computed tables)
-- 3. Properly identifies league games (same league + division)
-- 4. Handles ties for soccer
-- ============================================
...
-- Create the standings computation function
...
game_results AS (
  ...
  FROM games g
  JOIN team_lookup ht ON ht.school_id = g.home_team_id
  JOIN team_lookup at ON at.school_id = g.away_team_id
...
team_stats AS (
  ...
  SELECT
    ...
    COALESCE(SUM(
      CASE
        WHEN gr.home_team_id = tl.school_id AND gr.home_score > gr.away_score THEN 1
        WHEN gr.away_team_id = tl.school_id AND gr.away_score > gr.home_score THEN 1
        ELSE 0
      END
    ...
    COALESCE(SUM(
      CASE
        WHEN (gr.home_team_id = tl.school_id OR gr.away_team_id = tl.school_id)
    ...
    COALESCE(SUM(
      CASE
        WHEN gr.is_league_game AND gr.home_team_id = tl.school_id AND gr.home_score > gr.away_score THEN 1
        WHEN gr.is_league_game AND gr.away_team_id = tl.school_id AND gr.away_score > gr.home_score THEN 1
        ELSE 0
      END
    ...
    COALESCE(SUM(
      CASE
        WHEN gr.is_league_game
          AND (gr.home_team_id = tl.school_id OR gr.away_team_id = tl.school_id)
          AND gr.home_score = gr.away_score THEN 1
        ELSE 0
      END
    ...
    COALESCE(SUM(
      CASE
        WHEN gr.home_team_id = tl.school_id THEN gr.home_score
        WHEN gr.away_team_id = tl.school_id THEN gr.away_score
        ELSE 0
      END
    ), 0)::INT AS points_for,
    -- Points against
    COALESCE(SUM(
      CASE
        WHEN gr.home_team_id = tl.school_id THEN gr.away_score
        WHEN gr.away_team_id = tl.school_id THEN gr.home_score
        ELSE 0
      END
    ), 0)::INT AS points_against
  FROM team_lookup tl
  LEFT JOIN game_results gr ON gr.home_team_id = tl.school_id OR gr.away_team_id = tl.school_id
  GROUP BY tl.school_id, tl.league, tl.division, tl.region
)
...
COMMENT ON FUNCTION get_computed_standings IS 'Computes standings for a sport/gender/season using the teams table for proper league/division grouping. Returns all teams with their overall and league records.';
