...
export async function updateSession(request: NextRequest) {
  ...
  if (user && (!isPublicPath || isAdminPath || path === '/beta-landing')) {
    ...
    // Check ban status via separate function (can't alter get_user_permissions signature due to RLS policy deps)
    const { data: bannedData } = await supabase.rpc('is_user_banned', { p_user_id: user.id } as never)
    if (bannedData && !isPublicPath) {
